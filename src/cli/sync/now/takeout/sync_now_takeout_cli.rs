...
impl SyncNowTakeoutArgs {
    ...
    pub async fn invoke(self) -> eyre::Result<()> {
        ...
        let (watch_history_json, playlist_csvs, source_label) = if let Some(input_dir) = self.input_dir {
        ...
        for playlist_csv in &playlist_csvs {
            ...
            let mut entries = crate::takeout::read_playlist_video_entries(playlist_csv, playlist_name).await?;
            playlist_entries.append(&mut entries);
        }

        let watch_history_report = crate::takeout::read_watch_history_entries(&watch_history_json).await?;
        ...
        println!("watch-history-entry-count={}", watch_history_report.entries.len());
        ...
        println!("sync-unique-video-count={}", sync_summary.unique_video_count);
        println!("sync-unique-playlist-count={}", sync_summary.unique_playlist_count);
        println!("sync-playlist-event-count={}", sync_summary.playlist_event_count);
        println!("sync-watch-event-count={}", sync_summary.watch_event_count);
        println!("sync-written-event-file-count={}", sync_summary.written_event_file_count);
        println!(
...
fn discover_takeout_inputs_from_directory(input_dir: &Path) -> eyre::Result<(PathBuf, Vec<PathBuf>)> {
    let mut watch_history_candidates = Vec::new();
    let mut playlist_candidates = Vec::new();
    collect_takeout_files(input_dir, &mut watch_history_candidates, &mut playlist_candidates)?;
...
fn select_takeout_inputs(
    ...
    let watch_history_json = watch_history_candidates
        ...
        .ok_or_else(|| eyre::eyre!("Could not find a readable watch-history.json in the selected takeout inputs"))?;
...
fn candidate_timestamp_key(path: &Path) -> String {
    if let Some(root) = takeout_root(path) {
        if let Some(file_name) = root.file_name().and_then(std::ffi::OsStr::to_str) {
